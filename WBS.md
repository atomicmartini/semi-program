...
| 코드 | 산출물 | 완료 조건 | 선행 | 상태 |
| --- | --- | --- | --- | --- |
| 6.1 | `extract.py` | `python extract.py <기사>` → 카테고리·한국어 요약·관계가 정해진 형식으로 출력. **관계마다 원문 인용구 포함** | 5.1 | ⚠ 코드는 있으나 **모델 429 로 검증 못 함** |
| 7.1 | `link.py` | `python link.py <날짜>` → 오늘 사건이 과거 어느 사건의 후속인지 출력. 임계값 아래는 잇지 않음 | 3.2 | ✅ |
| 7.2 | `render.py` 의 흐름 화면 | 배포 URL 카드에 '이어지는 흐름' 이 펼쳐짐 | 7.1 | ✅ |
| 3.3 | `archive.py` · 과거분 | `python archive.py <시작달> <끝달> [사이트]` → 달마다 최대 5건씩 목록 페이지에서 받아 합침 | 4.1 | ✅ |
| 8.1 | `.github/workflows/daily.yml` | Actions 가 정해진 시각에 돌고 `docs/` 커밋이 쌓임. 실패 시 빨간불 | 7.1 | ☐ |

> `7.1` 은 `6.1` 을 기다리지 않고 끝냈다. 4절의 완화책("연결은 기업명 겹침만으로 판정")을
> 그대로 쓴 것이다 — **막힌 것 하나가 성공 판정 전체를 세우지 않게 한 장치가 실제로 작동했다.**
> `3.3` 은 원래 없던 작업이다. RSS 가 최근분만 줘서 흐름의 재료가 모자랐다.
...
| ~~지난 날짜 목록~~ | ✅ 2026-08-23 완료 | — |
| ~~필터 칩 동작~~ | ✅ 2026-08-23 완료 | — |
| ~~개념 탭~~ | ✅ 2026-08-23 완료 (용어 5개) | — |
...
| 2 | `6.1` 의 관계 추출 | 분류·요약만 하고 **관계는 뺀다.** 연결은 기업명 겹침만으로 판정 (세션 3 설계안, 지금은 슬라이스 11 로 바뀌었다 — `docs/slices/11-이어지는흐름-모델판정.md`) |
